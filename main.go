...
type RootCmd struct {
	...
	GitIgnore     []string              `help:"Specifies .gitignore files to use for filtering. When used without values, looks for a .gitignore in the root directory. Multiple files can be specified. This is merged with filter patterns, taking the same precedence." name:"gitignore"`
...
func (c *RootCmd) Run() error {
	...
	paths, err := internal.TraverseDirectory(c.Dir, c.Filter, c.GitIgnore)
	if err != nil {
